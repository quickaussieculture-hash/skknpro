...
export default function App() {
  ...
  const formatErrorMessage = (err: any): string => {
    ...
    if (msg.includes('429') || err.status === 'RESOURCE_EXHAUSTED' || msg.includes('quota') || msg.includes('RESOURCE_EXHAUSTED')) {
      return 'Hệ thống đang quá tải (Quota exceeded). Vui lòng đợi 1-2 phút hoặc sử dụng API Key trả phí để tiếp tục.';
    }
    
    // Check for API key errors
    if (msg.includes('API Key') || msg.includes('API_KEY_INVALID') || msg.includes('invalid api key')) {
    ...
    try {
      if (msg.startsWith('{')) {
        ...
        if (parsed.error?.message) {
          if (parsed.error.code === 429) return 'Hệ thống đang quá tải (Quota exceeded). Vui lòng đợi 1-2 phút.';
    ...
    return msg || 'Đã xảy ra lỗi không xác định. Vui lòng thử lại.';
  };
  ...
  return (
    <div className="min-h-screen tech-grid">
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <a 
              ...
              target="_blank" 
              rel="noreferrer" 
              className="bg-slate-100 text-slate-600 px-6 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 hover:bg-slate-200 transition-all border border-slate-200"
            >
      ...
      <main className="max-w-7xl mx-auto px-6 py-16 space-y-16">
        ...
        <section className="space-y-8">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-black text-slate-900 uppercase tracking-widest">Phân tích Tên đề tài</h2>
          </div>
          <div className="glass-card p-8">
            <div className="relative flex items-center">
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-6 px-8 text-slate-900 placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-500/10 transition-all outline-none text-lg"
              />
              <button 
                onClick={handleTitleAnalysis}
                disabled={isAnalyzingTitle || !title}
                className="absolute right-4 bg-indigo-600 text-white px-8 py-4 rounded-xl hover:bg-indigo-700 transition-all disabled:opacity-30 flex items-center gap-3 font-black text-sm shadow-xl shadow-indigo-500/20"
              >
            ...
            <AnimatePresence>
              {titleAnalysis && (
                <motion.div 
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="mt-10 pt-10 border-t border-slate-100 grid grid-cols-1 md:grid-cols-12 gap-12"
                >
                  <div className="md:col-span-3 flex flex-col items-center justify-center border-r border-slate-100">
                    <div className="relative">
                      <div className="absolute inset-0 blur-2xl bg-indigo-500/10 rounded-full" />
                      <span className="relative text-7xl font-black text-indigo-600 glow-text">{titleAnalysis.score}</span>
                    </div>
                    <span className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.3em] mt-4">Score / 10</span>
                  </div>
                  <div className="md:col-span-9 space-y-8">
                    <div className="space-y-3">
                      <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Nhận xét chuyên gia</h3>
                      <p className="text-slate-700 italic font-serif leading-relaxed text-xl">"{titleAnalysis.critique}"</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {titleAnalysis.suggestions.map((s, i) => (
                        <motion.div 
                          ...
                          className="p-5 bg-slate-50 rounded-2xl border border-slate-100 text-sm text-slate-600 flex items-start gap-4 group hover:bg-white hover:shadow-md transition-all relative overflow-hidden"
                        >
        ...
        <section className="space-y-8">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-black text-slate-900 uppercase tracking-widest">Tải lên Tài liệu (PDF/Word)</h2>
          </div>
          
          <div className="glass-card p-3">
            <label className="block w-full border-2 border-dashed border-slate-200 rounded-[28px] bg-slate-50 py-24 text-center cursor-pointer hover:bg-slate-100 transition-all group relative overflow-hidden">
              ...
              <div className="flex flex-col items-center gap-8 max-w-lg mx-auto px-6 relative z-10">
                <motion.div 
                  ...
                  className="w-24 h-24 bg-indigo-600 text-white rounded-3xl flex items-center justify-center border border-indigo-500/20 shadow-2xl shadow-indigo-500/10"
                >
                ...
                <div className="bg-indigo-600 text-white px-12 py-5 rounded-2xl font-black text-sm shadow-2xl hover:scale-105 transition-transform">
                  {isProcessingFile ? (
                    <div className="flex items-center gap-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  <div className="lg:col-span-8 glass-card p-12 flex flex-col md:flex-row gap-16">
                    <div className="w-full md:w-1/2 h-[350px] relative">
                      <div className="absolute inset-0 blur-3xl bg-indigo-500/5 rounded-full" />
                      <ResponsiveContainer width="100%" height="100%">
                        <RadarChart cx="50%" cy="50%" outerRadius="80%" data={radarData}>
                          <PolarGrid stroke="rgba(0,0,0,0.05)" />
                          <PolarAngleAxis dataKey="subject" tick={{ fontSize: 10, fontWeight: 800, fill: '#64748b', letterSpacing: '0.1em' }} />
                    ...
                    <div className="w-full md:w-1/2 flex flex-col justify-center space-y-10">
                      <div>
                        <div className="relative inline-block">
                          <div className="absolute inset-0 blur-2xl bg-indigo-500/10 rounded-full" />
                          <div className="relative text-8xl font-black text-slate-900 tracking-tighter glow-text">
                            {Object.values(deepReview.scores).reduce((a, b) => a + b, 0)}
                            <span className="text-3xl text-slate-400 font-bold">/100</span>
                          </div>
                        </div>
                        <p className="text-xs font-black text-indigo-600 uppercase tracking-[0.3em] mt-4">Tổng điểm thẩm định</p>
                      </div>
                      <div className="grid grid-cols-2 gap-12">
                        <div className="space-y-2">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">ĐẠO VĂN</p>
                          <p className={cn("text-4xl font-black glow-text", deepReview.plagiarism > 20 ? "text-red-500" : "text-emerald-600")}>
                            {deepReview.plagiarism}%
                          </p>
                        </div>
                        <div className="space-y-2">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">NGUY CƠ AI</p>
                          <p className={cn("text-4xl font-black glow-text", deepReview.aiRisk > 30 ? "text-orange-500" : "text-emerald-600")}>
                            {deepReview.aiRisk}%
                          </p>
                        </div>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <div className="md:col-span-2 glass-card p-12 space-y-12">
                    <div className="flex items-center gap-4">
                      ...
                      <h2 className="text-xl font-black text-slate-900 uppercase tracking-widest">Phân tích chuyên sâu</h2>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
                      <div className="space-y-5">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Cấu trúc</h4>
                        <p className="text-sm text-slate-600 leading-relaxed font-medium">{deepReview.deepReview.structure}</p>
                      </div>
                      <div className="space-y-5">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Sư phạm</h4>
                        <p className="text-sm text-slate-700 leading-relaxed italic font-serif text-lg">{deepReview.deepReview.pedagogy}</p>
                      </div>
                      <div className="space-y-5">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Minh chứng</h4>
                        <p className="text-sm text-slate-600 leading-relaxed font-medium">{deepReview.deepReview.data}</p>
                      </div>
                    </div>
                    ...
                    <div className="pt-12 border-t border-slate-100 space-y-8">
                      <div className="flex items-center gap-4">
                        ...
                        <h3 className="text-xl font-black text-slate-900 uppercase tracking-widest">Nội dung cần chỉnh sửa</h3>
                      </div>
                      <div className="grid grid-cols-1 gap-6">
                        {deepReview.improvementSuggestions.map((item, i) => (
                          <motion.div 
                            ...
                            transition={{ delay: i * 0.1 }}
                            className="bg-slate-50 rounded-3xl p-8 border border-slate-100 space-y-5 group hover:bg-white hover:shadow-lg transition-all"
                          >
                            <div className="flex items-center justify-between">
                              <span className="text-[10px] font-black text-orange-600 uppercase tracking-widest bg-orange-50 px-4 py-1.5 rounded-full border border-orange-100">
                                {item.section}
                              </span>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                              <div className="space-y-2">
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Vấn đề</p>
                                <p className="text-sm text-slate-600 font-medium leading-relaxed">{item.issue}</p>
                              </div>
                              <div className="space-y-2">
                                <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Cách sửa</p>
                                <p className="text-sm text-indigo-900 font-medium leading-relaxed">{item.fix}</p>
                              </div>
                            </div>
                          </motion.div>
                        ))}
                  ...
                  <div className="space-y-8">
                    <div className="glass-card p-10 space-y-8">
                      <div className="flex items-center gap-4">
                        ...
                        <h2 className="text-lg font-black text-slate-900 uppercase tracking-widest">Nguồn trùng lặp</h2>
                      </div>
                      <div className="space-y-4">
                        {deepReview.plagiarismSources.map((source, i) => (
                          <a 
                            ...
                            target="_blank" 
                            rel="noreferrer"
                            className="block p-5 bg-slate-50 rounded-2xl border border-slate-100 hover:border-indigo-500/30 hover:bg-white hover:shadow-md transition-all group"
                          >
                            <div className="flex items-center justify-between mb-3">
                              <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Tương đồng {source.matchPercentage}%</span>
                              <ChevronRight className="w-4 h-4 text-slate-400 group-hover:text-indigo-600 transition-colors" />
                            </div>
                            <p className="text-xs font-bold text-slate-700 line-clamp-2 group-hover:text-slate-900 transition-colors leading-relaxed">{source.title}</p>
                            <p className="text-[10px] text-slate-400 mt-2 truncate font-mono">{source.url}</p>
                          </a>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="glass-card p-10 space-y-8">
                      <div className="flex items-center gap-4">
                        ...
                        <h2 className="text-lg font-black text-slate-900 uppercase tracking-widest">Tham khảo</h2>
                      </div>
                      <div className="space-y-5">
                        {deepReview.references.map((ref, i) => (
                          <div key={i} className="flex gap-4 group cursor-pointer">
                            <span className="text-[10px] font-black text-indigo-600 group-hover:text-indigo-500 transition-colors mt-1 font-mono">0{i+1}</span>
                            <p className="text-xs text-slate-500 group-hover:text-slate-700 transition-colors leading-relaxed font-medium">{ref}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between relative z-10">
                      <div className="flex items-center gap-4">
                        ...
                        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Bản thảo đã tối ưu</h2>
                      </div>
                      <div className="flex gap-4">
                        <button 
                          onClick={() => navigator.clipboard.writeText(fixedContent)}
                          className="bg-white text-emerald-600 px-8 py-4 rounded-2xl text-xs font-black border border-emerald-200 hover:bg-emerald-50 transition-all tracking-widest shadow-sm"
                        >
                          SAO CHÉP
                        </button>
                        <button className="bg-emerald-600 text-white px-8 py-4 rounded-2xl text-xs font-black shadow-lg shadow-emerald-500/20 hover:bg-emerald-700 transition-all tracking-widest">
                          TẢI VỀ (.DOCX)
                        </button>
                      </div>
                    </div>
                    <div className="bg-white p-10 rounded-[32px] text-slate-700 font-serif leading-relaxed text-xl whitespace-pre-wrap max-h-[600px] overflow-y-auto border border-slate-200 shadow-inner relative z-10">
                      {fixedContent}
                    </div>
                  </motion.div>
                )}
      ...
      {/* Footer */}
      <footer className="bg-slate-100 border-t border-slate-200 pt-24 pb-16">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-12 gap-20">
          ...
          <div className="md:col-span-4 space-y-10">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">LIÊN HỆ</h3>
            <div className="space-y-5">
              <div className="bg-white p-5 rounded-3xl flex items-center gap-5 border border-slate-200 group hover:shadow-lg transition-all">
                ...
                <div>
                  <p className="text-sm font-black text-slate-900">Tác giả: Thầy Ksor Gé</p>
                </div>
              </div>
              <div className="bg-indigo-600 p-5 rounded-3xl flex items-center gap-5 border border-indigo-500 group hover:shadow-xl transition-all">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-indigo-100 font-black uppercase tracking-widest mt-1">Hỗ trợ kỹ thuật & Tư vấn</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-3 space-y-10">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">PHÁP LÝ & BẢO MẬT</h3>
            <ul className="space-y-5">
              <li><a href="#" className="text-sm font-bold text-slate-500 hover:text-indigo-600 transition-all flex items-center gap-3"><ChevronRight className="w-3 h-3" /> Điều khoản sử dụng</a></li>
              <li><a href="#" className="text-sm font-bold text-slate-500 hover:text-indigo-600 transition-all flex items-center gap-3"><ChevronRight className="w-3 h-3" /> Chính sách bảo mật</a></li>
              <li><a href="#" className="text-sm font-bold text-slate-500 hover:text-indigo-600 transition-all flex items-center gap-3"><ChevronRight className="w-3 h-3" /> Hướng dẫn Thông tư 27</a></li>
            </ul>
          </div>
        </div>
